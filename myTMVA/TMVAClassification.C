...
void TMVAClassification(TString collisionsyst="PbPb", float ptMin=7., float ptMax=10., TString myMethodList = "" )
{
  Bool_t isPbPb;
  ...
  signal->AddFriend("ntSkim");
   ...
   else cuts = Form("(%s)&&Dpt>%f&&Dpt<%f&&hiBin>=0&&hiBin<=200",mycuts.Data(),ptmin,ptmax);
   TString cutb; 
   if(!isPbPb) cutb = Form("(%s)&&Dpt>%f&&Dpt<%f",mycutb.Data(),ptmin,ptmax);
   else cutb = Form("(%s)&&Dpt>%f&&Dpt<%f&&hiBin>=0&&hiBin<=200",mycutb.Data(),ptmin,ptmax);
